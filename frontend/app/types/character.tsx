  ...
  const sync = async () => {
    ...
    for (const character of addedCharacters) {
      try {
        ...
        const allCharsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/characters/`);
    ...
    for (const updatedCharacter of updatedCharacters) {
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/characters/${updatedCharacter.name}/`, {
          method: 'PUT',
          headers: {
    ...
    for (const name of deletedCharacters) {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/characters/${name}/`, {
          method: 'DELETE',
        });
